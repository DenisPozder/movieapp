...
function EditMovie() {
    ...
    useEffect(() => {
        ...
        // if modal is false than reset cast
        if(modalOpen === false){
            setCast();
        }
        // if its success than reset form and navigate to addMovie
        if(isSuccess){
        ...
        <CastsModal modalOpen={modalOpen} setModalOpen={setModalOpen} cast={cast} />
        {
            ...
            <div className={sameClass} >
                ...
                <div className='flex flex-col gap-6'>
                        <h2 className='text-sl font-bold'>Edit "{movie?.name}"</h2>
                        <div className='w-full grid md:grid-cols-2 gap-6'>
                            <div className='w-full'>
                                <Input label='Movie Title' placeholder='Venom' type='text' name='name' register={register('name')} bg={true}  />
                                {
                                    errors.name && <InlineError text={errors.name.message} />
                                }
                            </div>
                            <div className='w-full'>
                                <Input label='Hours' placeholder='2hr' type='number' name='time' register={register('time')} bg={true}  />
                                {
                        ...
                        <div className='w-full grid md:grid-cols-2 gap-6'>
                            <div className='w-full'>
                                <Input label='Language Used' placeholder='English' type='text' name='language' register={register('language')} bg={true}  />
                                {
                                    errors.language && <InlineError text={errors.language.message} />
                                }
                            </div>
                            <div className='w-full'>
                                <Input label='Year of Release' placeholder='2019' type='number' name='year' register={register('year')} bg={true}  />
                                {
                        ...
                        <div className='full'>
                            <div className='text-sm w-full'>
                                <Select label='Movie Category' options={categories?.length > 0 ? categories : []} name='category' register={{...register('category')}} />
                        ...
                        {/* CASTS */}
                        <div className='w-full grid ld:grid-cols-2 gap-6 items-start'>
                            <div className='w-full'>
                                ...
                                <span className='text-border text-xs'>
                                    If you add new casts the prevoius casts will be deleted. So you should add them again
                                </span>
                            </div>
                            <div className='grid 2xl:grid-cols-4 lg:grid-cols-3 sm:grid-cols-4 grid-cols-2 gap-4'>
                                {
                                    casts?.length > 0 && casts?.map((user) => (
                                        <div key={user.id} className='p-2 italic text-xs text-text rounded flex-colo bg-main border border-border'>
                                            <img src={`${user?.image ? user.image : '/images/Logo.png'}`} alt={user.name} className='w-full h-24 object-cover rounded mb-2' />
                                            <p>{user.name}</p>
                        ...
                        {/* SUBMIT */}
                            <button disabled={editLoading} onClick={handleSubmit(onSubmit)} className='bg-subMain w-full flex-rows gap-6 font-medium text-white py-4 rounded'>
                                {
