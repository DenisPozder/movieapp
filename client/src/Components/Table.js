        ...
        <tr key={i}>
            <td className={`${Text}`}>
                <div className='w-12 p-1 bg-dry border border-border h-12 rounded overflow-hidden'>
                    <img className='h-full w-full object-cover' src={movie?.image ? movie?.image : '/images/Logo.png'} alt={movie?.name} />
                </div>
            </td>
            ...
            <td className={`${Text} float-right flex-rows gap-2`}>
                {admin ? (
                        ...
                        <>
                        <button onClick={() => DownloadVideo(movie?.video, movie?.name)} disabled={progress > 0 && progress < 100} className='border border-border bg-dry flex-rows gap-2 text-border rounded py-1 px-2'>
