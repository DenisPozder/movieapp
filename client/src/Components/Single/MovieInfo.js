...
function MovieInfo({movie, setModalOpen, DownloadMovieVideo, progress}) {
  return (
    <div className='w-full xl:h-screen relative text-white'>
        <img src={movie?.image ? movie?.image : "/images/Logo.png"} alt={movie?.name} className='w-full hidden xl:inline-block h-full object-cover' />
        <div className='xl:bg-main bg-dry flex-colo xl:bg-opacity-90 xl:absolute top-0 left-0 right-0 bottom-0'>
            <div className='container px-3 mx-auto 2xl:px-32 xl:grid grid-cols-3 flex-colo py-10 lg:py-20 gap-8'>
                <div className='xl:-col-span-1 w-full xl:order-none order-last h-header bg-dry border border-gray-800 rounded-lg overflow-hidden'>
                    <img src={movie?.titleImage ? movie?.titleImage : "/images/Logo.png"} alt={movie?.name} className='w-full h-full object-cover' />
                </div>
                <div className='col-span-2 md:grid grid-cols-5 gap-4 items-center'>
                    ...
                    <div className='col-span-2 md:mt-0 mt-2 flex justify-end'>
                        <button disabled={progress} onClick={() => DownloadMovieVideo(movie?.video, movie?.name)} className='md:w-1/4 w-full relative flex-colo bg-subMain hover:bg-transparent border-2 border-subMain transitions md:h-64 h-20 rounded font-medium'>
