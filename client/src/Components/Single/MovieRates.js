        ...
        <form onSubmit={handleSubmit(onSubmit)} className='xl:col-span-2 w-full flex flex-col gap-8'>
          ...
          {/* Submit */}
          {
            userInfo ? (
              ...
              <Link to='/login' className='bg-main border border-dashed border-border text-subMain py-4 w-full flex-colo rounded'>Login to review this movie</Link>
            )
          }
        </form>
        {/* Reviews */}
              ...
              movie?.reviews?.length > 0 ? movie?.reviews?.map((review) => (
                <div key={review?._id} className='md:grid flex flex-col w-full grid-cols-12 gap-6 bg-dry p-4 border border-gray-800 rounded-lg'>
                  <div className='col-span-2 bg-main hidden md:block'>
                    <img src={review?.userImage ? review?.userImage : '/images/Logo.png'} alt={review?.userName} className='w-full h-24 rounded-lg object-cover' />
                  </div>
